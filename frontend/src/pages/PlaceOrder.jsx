...
const PlaceOrder = () => {
    ...
    const getNewsletterDiscount = () => {
        // Newsletter discount only applies if NO voucher is applied
    ...
    const handleApplyVoucher = async () => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:8080/api/validate-voucher', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ code: voucherCode, userId: userId })
            });
            const result = await response.json();

            if (result.success) {
                setAppliedVoucher(result.voucher);
                toast.success(`Voucher "${result.voucher.code}" applied!`);
            } else {
    ...
    const onSubmitHandler = async (event) => {
        ...
        let orderData = {
            ...
            newsletterDiscountApplied: newsletterDiscount > 0 && !appliedVoucher,
    ...
    return (
        <div className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
            ...
            <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
                ...
                {/* Voucher Code Input */}
                <div className='mt-4'>
                    <label className='block text-[#504c41] font-medium mb-2'>Have a Voucher Code?</label>
                    {!appliedVoucher ? (
                        <div className='flex gap-2'>
                            <input
                                ...
                                className='flex-1 border border-gray-300 rounded py-2 px-3.5 outline-[#D0A823] uppercase'
                            ...
                            <button
                                ...
                                className='bg-[#D0A823] hover:bg-[#b8951f] disabled:bg-gray-300 disabled:cursor-not-allowed text-[#504C41] font-semibold px-6 py-2 rounded transition-colors'
                            >
                        ...
                        <div className='bg-green-50 border border-green-300 rounded p-3 flex items-center justify-between'>
                            <div>
                                <p className='text-green-700 text-sm font-semibold'>✓ Voucher Applied: {appliedVoucher.code}</p>
                                <p className='text-green-600 text-xs mt-1'>
                                    {appliedVoucher.discountType === 'percentage'
                                        ? `${appliedVoucher.discountValue}% discount`
                                        : `RM ${appliedVoucher.discountValue.toFixed(2)} off`}
                                </p>
                            </div>
                            <button
                                type='button'
                ...
                {/* Newsletter Discount Indicator - only show when no voucher applied */}
                {newsletterDiscount > 0 && !appliedVoucher && (
                ...
                {/* Show info if newsletter discount is available but voucher is applied */}
                {user?.isNewsletterSubscribed && !user?.hasUsedNewsletterDiscount && appliedVoucher && (
                    <div className='bg-blue-50 border border-blue-300 rounded p-3 mt-2'>
                        <p className='text-blue-700 text-xs'>ℹ️ Your newsletter discount is available but voucher takes priority</p>
                    </div>
                )}
